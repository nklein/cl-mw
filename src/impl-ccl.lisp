...
(defun parse-ld.so.cache (&key (program "/sbin/ldconfig") (args '("-p")))
  ...
  (let ((ht (make-hash-table :test #'equal))
        (lines (stream->string-list
                (out-stream)
                (ccl:run-program program args :output out-stream))))
...
(defun mw-dump-exec (&key (exec-name "./a.out") ignore-libs remap-libs)
  ...
  (let ((shlibs nil)
    ...
    (dolist (shlib ccl::*shared-libraries*)
      (with-accessors ((namestring ccl::shlib.pathname)
                       (soname ccl::shlib.soname)
                       (opencount ccl::shlib.opencount)) shlib
              ...
              (progn
                ...
                (setf namestring
                      (ccl:native-translated-namestring
                       (translate-logical-pathname new-path)))
                (setf soname namestring)
  ...
  (format t "####################################~%")
  (ccl:save-application exec-name :toplevel-function #'_init
